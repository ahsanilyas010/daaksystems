...
export function Ingestion() {
  ...
  async function onUpload(e: React.FormEvent) {
    ...
    try {
      ...
      await api.upload("/ingestion/batches", form);
      setFile(null);
      await refresh();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-gray-600">
        Upload a Shopify order-printer PDF bundle or a carrier airway bill. Every order in it gets
        extracted and staged here for review before anything lands in the shipment board.
      </p>

      <form onSubmit={onUpload} className="flex flex-wrap items-end gap-4 rounded border bg-white p-4">
        ...
        <div>
          <label htmlFor="ingestion-file" className="block text-sm font-medium text-gray-700">PDF file</label>
          <input
            id="ingestion-file"
            type="file"
